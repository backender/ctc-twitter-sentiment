...
class TwitterStreamer:
    ...
    def __init__(self, tracker, toFilter):
    ...
    def run(self, writer):
        latestItems=[]
        memorized=100
        for line in self.api.GetStreamFilter(track=self.tracker, languages=['en']):
...
#                print 'Found duplicate : ' + text
            ...
            try:
                # apply filters
                lowered=text.lower()
                if any(writer in lowered for writer in self.toFilter):
                    continue
